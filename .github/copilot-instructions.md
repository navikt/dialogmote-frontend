...
Next.js Pages Router app for employer and employee dialogmøte flows. Owned by Team eSyfo (`navikt`).

## Source map

`src/pages/` owns routes. `src/server/auth/` handles authentication, `src/server/data/` data access and mapping, and `src/observability/` runtime errors.
